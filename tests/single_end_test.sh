...
if [[ ! -f seqs_qc/SRR10759419_1_fastqc.html ]]; then
  >&2 echo "ERROR: FastQC files missing."
fi

if [[ ! -f fastqs/trimmed/SRR10759419_1.fastq || ! -f fastqs/trimmed/S288C_log.txt ]]; then
  >&2 echo "ERROR: Trimmed FASTQ files and/or trimming log missing."
fi

if [[ ! -f fastqs/trimmed_qc/SRR10759419_1_fastqc.html ]]; then
